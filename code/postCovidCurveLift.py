...
sorted_indices = np.argsort(y_pred_proba[:, model.classes_.tolist().index(1)])[::-1]
X_sorted = X[sorted_indices]
y_sorted = y[sorted_indices]

# Calculate the cumulative positive rate (CPR)
cpr = np.cumsum(y_sorted) / len(y_sorted)

# Calculate the cumulative proportion of the population (CPOP)
cpop = np.arange(len(y_sorted)) / len(y_sorted)

    # Plot the lift curve
RocCurveDisplay(fpr=cpop, tpr=cpr).plot()
plt.title("Lift Curve")
plt.xlabel("Cumulative Proportion of Population (CPOP)")
plt.ylabel("Cumulative Positive Rate (CPR)")
plt.show()